...
function App() {
  ...
  const handleNewChat = () => {
    if (currentMessages.length > 0) {
      setConversations(prev => [...prev, currentMessages])
    }
    setCurrentMessages([])
  }
  ...
  return (
    <div className="app-container">
        ...
        <div className="conversation-list">
          {conversations.map((conv, idx) => (
            <div key={idx} className="conversation-item">
              ...
              <span>{conv[0]?.content?.slice(0, 30) || 'Conversation'}...</span>
            </div>
          ))}
        </div>
      ...
      <main className="main-content">
        <ChatWindow messages={currentMessages} setMessages={setCurrentMessages} />
      </main>
    </div>
  )
}
